-- Migration: Add contact_number to beneficiaries in answers JSONB field
-- This script updates all existing beneficiaries to include a contact_number field with default value
...
BEGIN
  ...
  LOOP
    ...
    -- Loop through each beneficiary and add contact_number if missing
    FOR beneficiary IN
      SELECT * FROM jsonb_array_elements(will_record.answers->'beneficiaries')
    LOOP
      -- Add contact_number field if it doesn't exist
      IF NOT (beneficiary ? 'contact_number') THEN
        beneficiary := beneficiary || jsonb_build_object('contact_number', '123456789');
      END IF;
...
DO $$
BEGIN
  RAISE NOTICE 'Migration completed: Added contact_number to all beneficiaries';
END $$;
