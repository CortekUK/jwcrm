...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Triggering document threshold alerts Edge Function...');

    // Call the Edge Function
    const edgeFunctionResponse = await fetch(
      `${supabaseUrl}/functions/v1/send-document-threshold-alerts`,
      {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json({
      success: true,
      message: result.message || 'Threshold alert triggered successfully',
      result,
    }, { headers });

  } catch (error) {
    console.error('Error triggering threshold alert:', error);
