...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const type = body.type || 'monthly'; // 'monthly' or 'quarterly'
    ...
    const functionName = type === 'quarterly' 
      ? 'send-kpi-quarterly-notifications' 
      : 'send-kpi-monthly-notifications';

    console.log(`Triggering KPI ${type} reminder Edge Function: ${functionName}...`);
    ...
    const edgeFunctionResponse = await fetch(
      `${supabaseUrl}/functions/v1/${functionName}`,
      {
        ...
        body: JSON.stringify({ force: true }), // Force send even if not within normal window
      }
    );
    ...
    return NextResponse.json({
      success: true,
      message: `KPI ${type} reminder triggered successfully`,
      result,
    }, { headers });

  } catch (error) {
    console.error('Error triggering KPI reminder:', error);
