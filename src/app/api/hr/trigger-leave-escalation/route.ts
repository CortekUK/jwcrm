...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Triggering leave escalation Edge Function...');

    // Call the Edge Function
    const edgeFunctionResponse = await fetch(
      `${supabaseUrl}/functions/v1/process-leave-escalations`,
      {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json({
      success: true,
      message: 'Leave escalation processing triggered successfully',
      result,
    }, { headers });

  } catch (error) {
    console.error('Error triggering leave escalation:', error);
