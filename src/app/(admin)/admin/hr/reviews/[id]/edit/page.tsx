...
export default function AdminEditQuarterlyReviewPage() {
    ...
    const fetchReview = async () => {
      ...
      try {
        ...
        if (data.status !== "draft") {
          ...
          router.push(`/admin/hr/reviews/${reviewId}`);
          return;
        }
    ...
    <div className="space-y-6">
      ...
      <div className={`flex items-center gap-4 ${isRtl ? "flex-row-reverse" : ""}`}>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/admin/hr/reviews/${reviewId}`)}
      ...
      <QuarterlyReviewForm
        ...
        onSuccess={() => router.push(`/admin/hr/reviews/${reviewId}`)}
      />
    </div>
  );
}
