...
export default function AdminEditMonthlyReviewPage() {
    ...
    const fetchReview = async () => {
      ...
      try {
        ...
        if (data.status !== "draft") {
          ...
          router.push(`/admin/hr/reviews/monthly/${reviewId}`);
          return;
        }
        ...
        router.push("/admin/hr/reviews/monthly");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-6">
      ...
      <div className={`flex items-center gap-4 ${isRtl ? "flex-row-reverse" : ""}`}>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/admin/hr/reviews/monthly/${reviewId}`)}
      ...
      <MonthlyReviewForm
        ...
        onSuccess={() => router.push(`/admin/hr/reviews/monthly/${reviewId}`)}
      />
    </div>
  );
}
