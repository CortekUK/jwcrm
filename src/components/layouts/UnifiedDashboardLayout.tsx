        ...
        const navItem = (
          <Link key={item.path} href={item.path}>
            ...
            >
              ...
              {!isCollapsed && (
                <span className="font-medium text-[14px] truncate">{t(item.labelKey)}</span>
              )}
            </div>
          </Link>
        );
    ...
    >
      <div className="flex h-full flex-col">
        ...
        <div className={cn(
          ...
          isCollapsed ? "px-2 py-[26px]" : "py-8 px-4"
        )}>
          {isCollapsed ? (
        ...
        <div className={cn(
          ...
          <RoleSwitcher compact={isCollapsed} />
        </div>
...
export function UnifiedDashboardLayout({ children }: UnifiedDashboardLayoutProps) {
  return (
    <SidebarProvider layoutType="admin">
      <TooltipProvider>
