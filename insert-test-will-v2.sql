...
-- This version uses an existing user from your database

-- Step 1: Find an existing user (run this first to get a user_id)
SELECT id, email FROM auth.users LIMIT 5;

-- Step 2: Replace 'YOUR_USER_ID_HERE' below with an actual user_id from the query above
...
BEGIN
  ...
  VALUES (
    ...
    'draft_released',
    NOW() - INTERVAL '5 days',
    'wills/' || v_user_id::text || '/draft_1729084928000.pdf',
    NOW() - INTERVAL '2 days',
    TRUE,
    'disapproved',
    NOW() - INTERVAL '1 day',
    ...
    jsonb_build_array(
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/passport-1760616127700.jpg',
        'size', 245680,
        'type', 'passport',
      ...
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/emirates_id-1760616127701.jpg',
        'size', 198432,
        'type', 'emirates_id',
        'uploaded_at', NOW() - INTERVAL '5 days'
      ),
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/emirates_id-1760616127702.jpg',
        'size', 201567,
        'type', 'emirates_id',
        'uploaded_at', NOW() - INTERVAL '5 days'
      ),
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/document-1760616127703.pdf',
        'size', 456789,
        'type', 'document',
        'uploaded_at', NOW() - INTERVAL '5 days'
      ),
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/poa-1760616127705.jpg',
        'size', 112725,
        'type', 'poa',
        'uploaded_at', NOW() - INTERVAL '5 days'
      ),
      jsonb_build_object(
        ...
        'path', 'wills/' || v_user_id::text || '/document-1760616127706.pdf',
        'size', 678901,
        'type', 'document',
        'uploaded_at', NOW() - INTERVAL '5 days'
      )
  ...
  -- Insert status history events
  INSERT INTO public.will_status_events (will_id, previous_status, new_status, actor_user_id, notes, created_at)
  VALUES
    (v_will_id, 'in_progress', 'awaiting_review', v_user_id, 'Client completed the will form', NOW() - INTERVAL '5 days'),
    (v_will_id, 'awaiting_review', 'under_review', v_user_id, 'Admin started reviewing the submission', NOW() - INTERVAL '4 days'),
    (v_will_id, 'under_review', 'draft_ready', v_user_id, 'Draft completed and ready for release', NOW() - INTERVAL '2 days'),
    (v_will_id, 'draft_ready', 'draft_released', v_user_id, 'Draft PDF released to client for review', NOW() - INTERVAL '2 days');

  RAISE NOTICE 'Will created successfully with ID: %', v_will_id;
END $$;

-- View the inserted will
SELECT
  id,
  user_id,
  status,
